...
const saveTrabajador = async () => {
  ...
  try {
    ...
    if (isEditing.value) {
      ...
      const debugInfo = `keys:${Object.keys(res.data||{}).join('|')} / data:${JSON.stringify(actualizado).slice(0,80)}`
      toast.add({ severity: 'success', summary: 'Éxito', detail: debugInfo, life: 15000 })
    } else {
